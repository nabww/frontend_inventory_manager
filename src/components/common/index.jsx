...
export const Spinner = ({ size = 16 }) => (
  <RiLoader4Line size={size} className="spin" />
);
...
export const Modal = ({
  ...
  if (!open) return null;
  return (
    <div className="overlay" onClick={(e) => e.target === e.currentTarget}>
    ...
    footer={
      <>
        ...
        <button
          ...
          onClick={onConfirm}
          disabled={loading}>
          {loading ? <Spinner size={13} /> : <RiCheckLine size={13} />}{" "}
...
export const StatusBadge = ({ status }) => {
  const map = {
    active: "b-active",
    under_repair: "b-repair",
    decommissioned: "b-decomm",
    lost: "b-lost",
    pass: "b-pass",
    fail: "b-fail",
    partial: "b-partial",
  ...
  return (
    <span className={`badge ${map[status] || "b-decomm"}`}>
      {status?.replace(/_/g, " ")}
    </span>
  );
};
...
export const SimBadge = ({ hasSim }) => (
  <span className={`badge ${hasSim ? "b-sim" : "b-decomm"}`}>
    {hasSim ? "SIM" : "WiFi only"}
  </span>
);
...
export const Pagination = ({ page, totalPages, onChange }) => {
  ...
  return (
    <div className="pages">
      ...
      {pages[0] > 1 && (
        <>
          <button className="pg" onClick={() => onChange(1)}>
            1
          </button>
          <span className="dim" style={{ padding: "0 3px" }}>
            …
          </span>
        </>
      )}
      ...
      {pages[pages.length - 1] < totalPages && (
        <>
          ...
          <button className="pg" onClick={() => onChange(totalPages)}>
            {totalPages}
          </button>
        </>
      )}
      ...
      <tr key={i}>
        {Array.from({ length: cols }).map((_, j) => (
          <td key={j}>
            <div
              className="skeleton"
              style={{ height: 13, width: j === 0 ? 70 : "60%" }}
            />
          </td>
        ))}
      </tr>
